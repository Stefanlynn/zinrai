...
export default function Culture() {
  ...
  // Toggle between intro text and values view
  const toggleView = () => {
    setShowIntro(!showIntro);
  };

  // Handle value click to show details
  ...
  return (
    <div className="bg-black min-h-screen w-full overflow-hidden">
      ...
      <div className="w-full min-h-screen flex flex-col relative z-10">
        ...
        {/* Main content area with conditional rendering for intro or values */}
        <div className="absolute top-[20vh] bottom-[20vh] left-[10vw] right-[10vw] overflow-y-auto">
          {showIntro ? (
            /* Intro Text View */
              ...
              {cultureValues.map((value, index) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <span className="text-white text-xl font-bold w-8 h-8 flex items-center justify-center border border-white/30 rounded-full">
                      {value.letter}
                    </span>
                    <h2 className="text-white text-lg font-medium">{value.title}</h2>
                    {activeValue === index && (
                      <div className="ml-2">
                  ...
                  <div className={`mt-3 text-white/70 text-sm overflow-hidden transition-all duration-300 ${activeValue === index ? 'max-h-32 opacity-100' : 'max-h-0 opacity-0'}`}>
                    {value.description}
                  </div>
                </div>
              ))}
        ...
        {/* Toggle Button - Bottom Center - Moved up for better visibility */}
