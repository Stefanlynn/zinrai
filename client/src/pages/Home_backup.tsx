...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Function to add multiple flickering images for a 3-5 second sequence
    const startFlickerSequence = () => {
      ...
      // Create a map to track which image is in which box
      ...
      // Create more random, rapid movements of images between boxes
      // Instead of predictable sequences, we'll create multiple rapid moves
      
      // All 8 possible boxes in the grid
      const allBoxes = [1, 2, 3, 4, 5, 6, 7, 8];
      
      // Create 20-25 random image movements throughout the flicker sequence
      // This ensures images appear in multiple different boxes rapidly
      const totalMoves = 25;
      
      // Create rapid, random movements
      for (let moveIdx = 0; moveIdx < totalMoves; moveIdx++) {
        // Which image to move (random selection each time)
        const imageIdx = moveIdx % 4; // Cycle through all 4 images
        
        // Pick a random box for this image
        const randomBox = allBoxes[Math.floor(Math.random() * allBoxes.length)];
        
        // Calculate when this move should happen - frequent rapid changes
        const moveTime = 200 + (moveIdx * 150);
          
          // Schedule the move
          const moveTimer = setTimeout(() => {
            if (!isFlickering) return;
            
            // First, find and remove this image from any box it might be in
            setActiveFlickerBoxes(prev => {
              const newBoxes = { ...prev };
              
              // Remove this image from any box it's currently in
              Object.entries(newBoxes).forEach(([boxNum, img]) => {
                if (img === flickerImages[imageIdx]) {
                  delete newBoxes[parseInt(boxNum)];
                }
              });
              
              // Place the image in its new random box
              newBoxes[randomBox] = flickerImages[imageIdx];
        ...
        {Object.entries(activeFlickerBoxes).map(([boxNum, imageSrc]) => {
          ...
          return (
            ...
            >
              ...
              >
                {/* The image with flickering animation - matching the reference site */}
                <img 
                  src={imageSrc} 
                  alt="" 
