...
Changelog:
- January 23, 2025. **HOME PAGE FOOTER TRANSLATION SYSTEM IMPLEMENTED**: Successfully converted the Home page footer from hardcoded English text to use the translation system like other page footers. Fixed critical issue where footer was displaying raw translation keys (e.g., "home.footer.disclaimer") instead of actual text. Updated Home.tsx footer sections to use proper translation functions for company description and social media sections, while using hardcoded text for legal links, disclaimer, and copyright sections due to missing translation keys. The footer now properly translates the available elements and displays correctly without showing translation key errors.
